...
function RouteComponent() {
   ...
   const { isPending: isMessagesPending, isError: isMessagesError } = handleMessagePaginataion({ roomId, setMessages }) // TODO handle loading and error states
   ...
   useEffect(() => {
      ...
      const cleanup = useSockets({
         'receive-message': (message: MessageType) => {
            setMessages((draft) => {
               draft.push({ data: message, isLocal: false })
            })
         ...
         'participant-message-edited': (messageId: string, text: string) => {
            handleMessageEdit(messageId, text)
         },
         'participant-message-deleted': (messageId: string) => {
   ...
   function handleMessageEdit(messageId: string, text: string) {
      setMessages((d) => {
         const i = d.findIndex((msg) => !msg.isLocal && msg.data.id == messageId)
   ...
   // TODO maybe context/reducer for this
   const {
      emit: msgReportEmit,
      isPending: isMsgReportPending,
      error: msgReportError,
      clearError,
   } = useSocketEmit('message-report', 'Failed to report message')
   function handleMessageReport(messageId: string) {
      if (confirm('Are you sure you want to report this message? Reason: other')) {
         msgReportEmit({ id: messageId, reason: 'other' })
      }
   }
   ...
   return (
      <div>
         <header className="fixed top-0 left-0 w-full bg-white z-10 px-2 py-0.5 border-b border-gray-300 shadow-sm flex items-center justify-between">
            ...
            {msgReportError && (
               <span className="text-xs text-red-500" onClick={() => clearError()}>
                  {msgReportError.message}
               </span>
            )}
         </header>
         <main className="w-full flex flex-col p-4 overflow-x-hidden mb-15 mt-10">
            ...
            <div>
               {Object.entries(
                  Object.groupBy(messages, (msg) => new Date(msg.data.createdAt).toDateString().split(' ').slice(1, 3).join(' ')),
               ).map(([date, messages], i) => (
                  <div id="messages" className="flex flex-col gap-2 w-full flex-1" key={i}>
                     ...
                     {messages!.map((msg, j) => {
                        if (msg.isLocal) {
                           ...
                           return <Message key={j} message={msg.data} onReport={() => handleMessageReport(msg.data.id)} />
                        }
                     })}
                  </div>
               ))}
         ...
         <footer className="fixed bottom-0 left-0 w-full shadow-md">
            {!msgEditing ? (
               <MessageTextField userId={me.id} roomId={roomId} setMessages={setMessages} setScrollTrigger={setScrollTrigger} />
            ) : (
